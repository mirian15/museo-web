...
prevSala.addEventListener('click', ()=> {
  currentIndex = (currentIndex - 1 + SALAS.length) % SALAS.length;
  renderModal();
});
nextSala.addEventListener('click', ()=> {
  currentIndex = (currentIndex + 1) % SALAS.length;
  renderModal();
});
